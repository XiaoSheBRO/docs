...
超文本标记语言（_HyperText Markup Language_），用于定义网页内容。
...
`<br>` 是 HTML4 写法；`<br />` 是 XHTML 为兼容 HTML 的写法，也是 XML 写法；HTML5 因为兼容 XHTML，所以两种写法都可以使用。
...
HTML5 中一些属性声明时可以省略属性值或直接使用属性名作为属性值。
...
根元素：一个页面最多存在一个根元素，且 `<html>` 是所有其他元素的父元素或祖先元素。

> HTML5 规范中， `<html>` 作为根元素不具有强制性。

`lang` 属性：**全局属性**，用于定义元素内容的自然语言。

::: tip 简体中文最新表示

~~`lang="zh-CN"`~~ ==> `lang="cmn-hans"`
...
用于链接外部资源（_CSS、JavaScript、图标等_）

```html
<head>
  <!-- 链接外部样式表 -->
  <link rel="stylesheet" type="text/css" href="style.css" />
  <!-- 外部网站图标；网站图标也可直接命名为 favicon.ico 放在网站根目录下 -->
  <link rel="icon" type="image/x-icon" href="favicon.ico" />
</head>
```

- `rel` _relationship_ 描述当前文档与被链接的资源之间的关系
  1. `stylesheet` 样式表
  2. `icon` 网站图标
- `type` 描述被链接资源的 MIME 类型；可省略
- `href` 被链接资源的路径
...
2. 所有元素与展示效果无关（_元素只决定含义，展示由 CSS 决定_），**应该根据内容的含义选择要使用的元素**。
...
1. 为了搜索引擎优化（_SEO_），使搜索引擎更容易理解网页内容，从而提高网站排名。
2. 为了让浏览器理解网页，增强浏览器使用体验。

## 元素的包含关系

> HTML5 以前：~~块级元素~~独占一行，~~行级元素~~不换行；~~块级元素~~可以包含~~行级元素~~，~~行级元素不~~能包含~~块级元素~~；`<a>` 元素除外。

HTML5 中：为了语义化，**元素的包含关系由元素的内容类别决定**。
...
- `<span>` 无语义元素，仅用于设置样式
- `<pre>` 预格式化文本元素
  - 内部的内容会按照源代码格式显示，不会出现[空白折叠](#空白折叠)
...
1. 跳转地址: `href="https://www.baidu.com"`
2. 跳转锚点: `href="#id"` 跳转到页面中 id 为 `id` 的元素位置；`href="#"` _空链接_，返回顶部
3. 功能链接: 点击后触发功能
   - 触发 JS 代码: `href="javascript:alert('Hello World')"`
   - 发送邮件: `href="mailto:123456789@qq.com"`，要求用户安装邮箱客户端软件
   - 拨打电话: `href="tel:12345678910"`，要求用户安装拨号软件
...
为了 SEO 优化，当使用图片作为超链接时，通常也会写上文本来描述链接的功能：
...
- `src` _source_ 图片资源地址
- `alt` _alternate_ 图片的替代文本（_当图片无法显示时显示_）
...
- `src` 视频资源地址
- `controls` 布尔属性；显示控件
- `autoplay` 布尔属性；自动播放；**某些浏览器中不支持非静音自动播放**
- `muted` 布尔属性；静音播放
- `loop` 布尔属性；循环播放
- `poster` 视频封面地址
- `width` 视频宽度
...
1. `<video>` 元素中嵌入提示或 flash 播放器。
2. 使用子元素 `<source>` 的 `src` 属性用于指定不同格式的视频资源。
...
当表示一个无顺序的列表时使用，**和显示序号的效果无关**；常用于制作菜单、新闻列表等。
...
**`<ol>`, `<ul>` 的子元素只能是 `<li>` 元素（_list item_）**，表示列表项。

:::

#### `<dl>` 定义列表 _definition list_

常用于术语定义

- `<dt>` _definition title_ 定义标题
- `<dd>` _definition description_ 定义描述
...
表示一块区域：该元素内部用于放置其他元素

- `<div>` 无语义元素
- `<header>` 通常用于表示页头或文章的头部
- `<footer>` 通常用于表示页脚或文章的尾部
- `<article>` 通常用于表示文章的正文或整篇文章
- `<section>` 通常用于表示文章中的一个区域，如文章中的一节
- `<aside>` 通常用于表示附加信息，如页面的侧边栏

### 嵌入元素

#### `<iframe>` 元素

可替换元素；通常用于在一个页面中嵌入另一个页面；**当前页面的 CSS 不能控制嵌入页面的样式。**
...
- `<object>` 可替换元素，用于引入外部资源
  - `data` 嵌入资源的路径
  - `type` 嵌入文件的类型(_MIME类型_)
  - 子元素：`<param name="" value="" />` 用于传递参数；`name` 属性表示参数名，`value` 属性表示参数值
- `<embed>` 可替换元素，用于引入外部资源
  - `src` 嵌入资源的路径
  - `type` 嵌入文件的类型(_MIME类型_)
...
- `type` 输入框类型
  ...
  - `date` 日期选择框，有兼容性问题
  - `file` 选择文件
  - `search` 搜索框；一般用在移动端；有兼容性问题
  - `range` 滑块；可同时设置 `min`, `max` 属性；有兼容性问题
- `value` 输入框的值
- `placeholder` 提示文本（_文本框没有内容时显示_）
- `maxlength` 最大字符数
- `min` 最小值
- `max` 最大值
- `step` 步长
...
- 子元素：`<option>` 表示选项
  - `value` 选项值
  - `selected` 布尔属性；表示默认选中
- 子元素：`<optgroup>` 表示选项分组（不能选中）
  - `label` 分组标题
...
数据列表；不会显示到页面；通常用于与普通文本框配合使用，提供输入提示；有兼容性问题
...
- `action` 提交路径；默认为当前页面地址
- `method` 提交方式

::: important

- 若要提交表单必须设置表单元素 `name` 属性，不设置会被丢弃。
- 提交的值为 `value` 属性。
...
## HTML 实体 _HTML Entity_

又叫实体字符；用于在页面中显示一些特殊符号。

### 空白折叠

在 HTML 源码中，连续多个空白符（_空格、制表符、换行符_）会被自动合并为一个空白符。
...
**绝对路径**：`协议名`://`主机名`:`端口号`/`路径`（_`scheme`://`host`:`port`/`path`_）
...
- 减少浏览器请求次数：减少了请求中浪费的时间
- 有利于动态生成数据

劣势：

- 增加了资源的体积，导致传输内容增加，从而增加了单个资源的传输时间。
- 不利于浏览器缓存（_浏览器通常会缓存图片，CSS, JS 等资源_）
- 会增加原资源的体积（_base64 编码后增加 1/3_）
...
一种编码方式，通常用于将一些二进制数据转变为用字符串可书写的形式。
