...
执行过程：将表达式 1 进行 Boolean 判定，若表达式 1 判定结果为假，则直接返回表达式 1 的结果，而不执行表达式 2；若表达式 1 为真，则返回表达式 2 的结果（_短路规则_）
...
执行过程：对表达式 1 进行 Boolean 判定，若为真，则返回表达式 2 的结果；若为假，则返回表达式 3 的结果
...
- JS 中表示的整数不是连续的
  - 当数字特别大时不再连续
- JS 中能表示的有效位数约 `16 ~ 17` 位

==在对精度要求很高的系统中，或要对小数运算结果进行比较时，需要特别谨慎==

JS 中十进制的小数，转换为二进制后，可能是无限小数，但是计算机对数字的存储能力有限，因此会丢失一些数据
...
- JS 能表示的最大安全整数（_最大连续整数_）：`Number.MAX_SAFE_INTEGER` 即 `0 10000110011 111...111` 即 `2^53 - 1`
...
在 JS 中，如果对一个数据进行位运算，他会将其转换为一个整数（_直接抹去小数部分_），并且按照 **32 位**的二进制格式进行排列，然后运算
...
将整数的二进制位向左移动指定位数（_即 `* 2 ^ n`_）

#### 右移运算 `>>`

<整数> `>>` <位数>

将整数的二进制位向右移动指定位数（_即 `/ 2 ^ n` 并取整_）
...
对条件表达式进行[Boolean判定](#逻辑运算符)，若为真，则执行代码块；

1. **如果某个条件判定为真则忽略后面所有条件**
2. `else if` 可以有多个，也可以没有
3. `else` 可以有一个，也可以没有
4. 如果代码块只有一条语句则可以省略 `{}`
