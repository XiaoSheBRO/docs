...
- 当读取的属性值不存在时（_属性值为 `undefined` 或 `null`_），会报错
...
**JS 中存在变量声明提升**
**所有**变量的**声明**会自动提升到代码最顶部

JS 中允许声明多个同名变量，**声明提升后声明会变为一个**
...
算数运算符（_除 `+` 外_）遇到非数字的类型时，会将其转换为数字类型（_自动完成转换_），然后进行运算；
...
- 通过函数字面量声明的函数（_如上_）会自动提升到脚本块的顶部
- 函数字面量会成为全局对象的属性
