...
判断对象是否有指定**属性名**，返回 `true` / `false`；**属性名为字符串**
...
取出起始下标到结束下标（_不包含_）的元素，并返回一个**新数组**；不会改变原数组；
...
从数组中查找指定元素的第一个下标（_严格相等_）并返回，如果不存在，则返回 `-1`

```js
arr.lastIndexOf(元素)
```

从数组中查找指定元素的最后一个下标（_严格相等_）并返回，如果不存在，则返回 `-1`
