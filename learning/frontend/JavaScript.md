...
算数运算符（_除 `+` 加法外_）遇到非数字的类型时，会将原始类型转换为数字类型（_自动完成转换_）,然后进行运算；转换规则：
...
1. 加号其中一边为 `string` 类型：进行字符串拼接(_将另一边的其他类型转为字符串_)
   - 数字 ==> 数字字符串
   - `null` ==> `"null"`
   - `undefined` ==> `"undefined"`
   - `boolean` ==> `"true"` / `"false"`
   - `object` ==> `"[object Object]"`
2. 加号两边都没有字符串但有对象：将对象转为字符串再进行字符串拼接
