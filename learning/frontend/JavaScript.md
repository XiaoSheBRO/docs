...
JavaScript 中，函数参数始终是按值传递的。
当参数是基本类型时，传递的是值本身；
当参数是引用类型时，传递的是对象引用这个“值”。
因此，给参数重新赋值不会影响外部变量，但通过该引用修改对象的属性会影响原对象；如果给对象赋值为新的对象，改变的是函数内部参数的引用，外部对象的引用地址不受影响。
...
- 函数调用完成后内部环境中的变量会被销毁
- **函数作用域由定义位置决定，不由调用位置决定**

::: important

JavaScript 是词法作用域语言（在 JavaScript 中，函数在创建时会记录其外层词法环境 _Lexical Environment_，形成固定的作用域链。），函数的作用域由其定义位置静态决定，而非运行时的调用位置；区别于 `this` 指向。
