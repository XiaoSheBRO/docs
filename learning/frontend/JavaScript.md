...
1. `else if` 可以有多个，也可以没有
2. `else` 可以有一个，也可以没有
3. 如果代码块只有一条语句则可以省略 `{}`
4. **如果某个条件判定为真则忽略后面所有条件**
...
计算表达式的返回值，依次和 `case` 后的数据进行**严格相等**的比较，如果某个数据相等，停止比较，然后运行其内部的代码块，然后依次运行之后的**所有代码块**。
...
- `new Array(长度)` 创建指定长度的空数组
  - 长度：数组的数据总数，非负整数
- `new Array(元素1, 元素2, ...)` 创建数组并初始化元素
- `[元素1, 元素2, ...]` 创建数组并初始化每一项的值
