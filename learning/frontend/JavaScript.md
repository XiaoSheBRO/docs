...
算数运算符（_除 `+` 加法外_）遇到非数字的类型时，会将其转换为数字类型（_自动完成转换_）,然后进行运算；转换规则：
...
如果一边不是字符串，且两边都为原始类型：转换为数字比较；[#转换规则](#非数字类型使用算术运算符)

> `NaN` 与其他任何数字比较，结果为 `false`。
> `Infinity` 比任何数字都大
> `- Infinity` 比任何数字都小

如果其中一个是对象：将对象转换为原始类型再进行比较
...
两端的类型相同直接比较数据本身（两个对象比较对象地址）
两端类型不同：

- `null` 和 `undefined` 之间相等和其他类型不相等
- 其他原始类型比较时，转换为数字比较
- `NaN` 与任何数字都不相等
- `Infinity` / `-Infinity` 只与自身相等
- 一边为对象比较时先转换为原始类型再比较

> 通常不使用 `==` 进行相等比较，因为 `==` 比较违反直觉
